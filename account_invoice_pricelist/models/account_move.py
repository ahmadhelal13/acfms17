...
class AccountMove(models.Model):
    ...
    patient_price = fields.Float("Patient Share", compute='_func_patient_and_insurance_price', store=True)
    patient_percentage = fields.Float(compute='_func_patient_and_insurance_price', store=True)
    insurance_price = fields.Float('Insurance Share', compute='_func_patient_and_insurance_price', store=True)

    @api.depends("invoice_line_ids", "pricelist_id", "price_total")
    def _func_patient_and_insurance_price(self):
        for rec in self:
            rec.insurance_price = (rec.price_total * rec.pricelist_id.percentage)/100
            rec.patient_price = (rec.price_total * (100 - rec.pricelist_id.percentage))/100
            rec.patient_percentage = 100 - rec.pricelist_id.percentage
    ...
    def _set_pricelist_currency(self):
        if self.is_invoice() and self.pricelist_id and self.currency_id != self.pricelist_id.currency_id:
            self.currency_id = self.pricelist_id.currency_id
        self.button_update_prices_from_pricelist()
