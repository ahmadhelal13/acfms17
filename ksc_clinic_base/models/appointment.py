...
class KscAppointment(models.Model):
    ...
    patient_id = fields.Many2one("res.partner", domain="[('is_patient','=',True)]", ondelete="restrict", required=True, index=True, help="Patient Name", tracking=True)
    physician_id = fields.Many2one(
        "res.partner", domain=lambda self: self._get_physician_domain(), ondelete="restrict", string="Physician", index=True, help="Physician's Name", tracking=True
    )
    ...
    invoice_id = fields.Many2one("account.move", string="Invoice", copy=False, ondelete="restrict")
    is_invoiced = fields.Boolean()
    services_invoice_id = fields.Many2one("account.move", string="Services Invoice", copy=False, ondelete="restrict")
    ...
    appointment_purpose = fields.Selection(
        [("consultation", "Consultation - كشف"), ("follow_up", "Follow Up - متابعة")], string="Appointment Purpose", default="consultation", tracking=True
    )
    start_date = fields.Datetime(string="Start Date", copy=False, default=lambda self: datetime.now() - timedelta(hours=3))
    end_date = fields.Datetime(string="End Date", copy=False, default=lambda self: datetime.now() - timedelta(hours=3, minutes=-20))
    room_id = fields.Many2one("ksc.room", domain=lambda self: self._get_room_domain(), ondelete="cascade", string="Room", help="Appointment Room", copy=False)
    ...
    company_id = fields.Many2one("res.company", ondelete="restrict", string="Hospital", default=lambda self: self.env.user.company_id.id)
    # =============== used for showing pricelist==================
    # diseases_ids = fields.One2many(
    #     'ksc.diseases', 'appointments_id',
    diseases_ids = fields.One2many("ksc.diseases.line", "appointments_id")

    differencial_diagnosis = fields.Text(
        string="Differential Diagnosis", help="The process of weighing the probability of one disease versus that of other diseases possibly accounting for a patient's illness."
    )
    medical_advice = fields.Text(
    ...
    invoice_state = fields.Selection(
        [("paid", "Paid - تم الدفع"), ("not_paid", "Not Paid - لم يتم الدفع"), ("partial", "Partially Paid - دفع جزئي")], compute="_compute_invoice_state"
    )
    has_credit_note = fields.Selection(
        [("has_credit", "Has Credit - تحتوي على مرتجع"), ("has_no_credit", "Has No Credit Note - لا تحتوي على مرتجع")], compute="_compute_has_credit_note"
    )

    hide_based_on_group = fields.Boolean(compute="_compute_hide_based_on_group")
    queue_number = fields.Integer()

    price_list_type = fields.Many2many("product.pricelist", string="Price Lists")

    # ===========================medical history=========================
    heart_disease = fields.Selection(related="patient_id.heart_disease", string="Heart disease - أمراض قلبية")
    bleeding = fields.Selection(related="patient_id.bleeding", string="Bleeding - النزف")
    hypertension = fields.Selection(related="patient_id.hypertension", string="Hypertension - أرتفاع")
    diabetes = fields.Selection(related="patient_id.diabetes", string="Diabetes - السكري")
    asthma = fields.Selection(related="patient_id.asthma", string="Asthma - الربو ")
    thyroid_disease = fields.Selection(related="patient_id.thyroid_disease", string="Thyroid disease - أمراض الغده الدرقية")
    rheumatoid_arthritis = fields.Selection(related="patient_id.rheumatoid_arthritis", string="Rheumatoid arthritis - التهاب مفاصل رثيوي")
    others_diseases = fields.Text(related="patient_id.others_diseases", string="Others - أمراض اخرى")

    do_you_take_any_medicine_now = fields.Selection(related="patient_id.do_you_take_any_medicine_now", string="Do you take any medicine now ? هل تأخذ أدويه حاليا؟ ")
    are_you_allergic_to_any_medicine = fields.Selection(related="patient_id.are_you_allergic_to_any_medicine", string="Are you allergic to any medicine ? هل لديك حساسيه للأدوية")
    others_drugs = fields.Text(related="patient_id.others_drugs", string="Others - أدوية اخرى")
    for_ladies = fields.Selection(related="patient_id.for_ladies", string="For ladies - للسيدات")
    remarks = fields.Text(related="patient_id.remarks", string="Remarks - ملاحظات ")
    extra_oral_exam = fields.Text(related="patient_id.extra_oral_exam", string="Extra Oral Exam")
    intra_oral_exam = fields.Text(related="patient_id.intra_oral_exam", string="Intra Oral Exam")
    x_ray_investigation = fields.Text(related="patient_id.x_ray_investigation", string="X-Ray Investigation")
    periaplcal = fields.Text(related="patient_id.periaplcal", string="Periaplcal")
    occlusal = fields.Text(related="patient_id.occlusal", string="Occlusal")
    ...
    def set_values_for_price_list(self):
        ...
        invoice_ids = []
        for rec in self:
            invoice_ids = rec.env["account.move"].search([("appointment_name", "=", rec.name), ("clinic", "=", rec.get_clinic_name())])
            if invoice_ids:
    ...
    def get_queue_number(self):
        ...
        prev_app_id = self.env[f"ksc.{self.get_clinic_name()}.appointment"].search(
    ...
    read_based_on_nurse_group = fields.Boolean(compute="_compute_read_based_on_nurse_group")
    ...
    read_based_on_manager_group = fields.Boolean(compute="_compute_read_based_on_manager_group")
    ...
    read_based_on_doctor_group = fields.Boolean(compute="_compute_read_based_on_doctor_group")
    ...
    def get_clinic_name(self):
        name = self._name.replace("ksc.", "").replace(".appointment", "").replace(".", "_")
        return name
    ...
    def view_invoice(self):
        appointment_invoices = self.env["account.move"].search([("id", "=", self.invoice_id.id)])
    ...
    def action_open_prescription(self):
        action = self.env["ir.actions.actions"]._for_xml_id("ksc_prescription_model.ksc_prescription_mode_ksc")
        action["context"] = {
    ...
    def action_open_radio_request(self):
        action = self.env["ir.actions.actions"]._for_xml_id("ksc_radiology.action_ksc_radiology_consultation")
        action["context"] = {
            "default_patient_id": self.patient_id.id,
            "default_physician_id": self.physician_id.id,
            "default_date": self.start_date,
    ...
    def is_follow_up(self):
        ...
        if self.appointment_purpose == "follow_up":
            ...
            if not self.special_follow_up:
                self.product_id = None
                b = {"domain": {"product_id": [(available, "=", 1), ("clinical_type", "=", "follow_up")]}}
            if self.patient_id and self.physician_id and self.special_follow_up:
                last_consultation = self.search(
                    [("patient_id.id", "=", self.patient_id.id), ("physician_id.id", "=", self.physician_id.id), ("state", "not in", ["draft", "cancel"])],
                ...
                if last_consultation:
                    ...
                    if last_consultation.state != "done":
                        ...
                        raise UserError(_("Last consultation {} not done".format(last_consultation.name)))
                    if last_consultation.appointment_purpose == "consultation":
                        b = {
                            "domain": {
                                "product_id": [(available, "=", 1), ("day_from", "<=", delta.days), ("day_to", ">", delta.days), ("clinical_type", "=", "special_follow_up")]
                            }
                        }
                    else:
                    ...
                    raise UserError(_("This Patient Has No Appointment With Doctor Before"))
            ...
            self.product_id = None
            b = {"domain": {"product(_id": [(available, "=", 1), ("clinical_type", "=", "consultation")]}}
        return b
    ...
    def print_report(self):
        ...
        payment_ids = self.env["account.payment"].search([("clinic", "=", clinic_name), ("date", "=", fields.Date.today()), ("create_uid", "=", self.env.uid)])
        if payment_ids:
...
class KSCDiseases(models.Model):
    ...
    name = fields.Char(string="Name", required=True, help="Disease name", default="New Disease", index=True)
    ref = fields.Char(string="Disease Id", readonly=True, copy=False, tracking=True)
    ...
    def create(self, values):
        if values.get("ref", "New Disease") == "New Disease":
            values["ref"] = self.env["ir.sequence"].next_by_code("ksc.diseases") or "New Disease"
