...
class kscPatientEvaluation(models.Model):
    ...
    bmi = fields.Float(compute="get_bmi_data",
                       string='Body Mass Index', store=True)
    ...
    def _constrains_systolic_bp_diastolic_bp(self):
        for rec in self:
            if rec.systolic_bp and len(str(rec.systolic_bp)) > 3:
                raise ValidationError(
                    'Systolic BP must be at most 3 digits long.')
            if rec.diastolic_bp and len(str(rec.diastolic_bp)) > 3:
                raise ValidationError(
