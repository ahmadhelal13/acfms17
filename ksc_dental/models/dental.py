...
class ProductProduct(models.Model):
    ...
    is_treatment = fields.Boolean(
        'Treatment', help="Check if the product is a Treatment")
        ...
        for each_rec in all_records:
            if each_rec.treatment == True:
                treatment_list.append(
                    {'treatment_categ_id': each_rec.id, 'name': each_rec.name, 'treatments': []})
            ...
            for each_treatment in treatment_list:
                if each_template.categ_id.id == each_treatment['treatment_categ_id']:
                    each_treatment['treatments'].append(
                        {'treatment_id': each_product.id, 'treatment_name': each_template.name, 'action': each_product.action_perform})
                    break
...
class MedicalTeethTreatment(models.Model):
    ...
    description = fields.Many2one('product.product', 'Description', domain=[
                                  ('is_treatment', '=', True)])
    ...
    teeth_code_rel = fields.Many2many(
        'teeth.code', 'teeth_code_medical_teeth_treatment_rel', 'operation', 'teeth')
    ...
    compute_date_create = fields.Datetime(
        compute='_compute_date_create', string='Create Date')
    ...
    def unlink(self):
        for rec in self:
            if rec.state in ('completed', 'in_progress') and not self.env.user.has_group(
                    'ksc_dental.group_allow_delete_teeth_treatment'):
                raise UserError(
                    _('You cannot remove/deactivate an (In Progress , Completed) record'))
...
class OrthodonticPlan(models.Model):
    ...
    physician_id = fields.Many2one(
        'res.partner', domain=[('is_physician', '=', True)], required=True)
    product_id = fields.Many2one('product.product', domain=[
                                 ('is_treatment', '=', True)], required=True)
    ...
    def unlink(self):
        for rec in self:
            if rec.invoice_id:
                raise UserError(
                    _('You cannot remove/deactivate an invoiced record'))
    ...
    def create_invoice(self):
        ...
        invoice = self.ksc_create_invoice(
            partner=self.patient_id, product_data=products_data, inv_data=inv_data)
        self.invoice_id = invoice.id

        check_duplicate = self.env['account.move'].search(
            [('partner_id', '=', self.patient_id.id), ('invoice_line_ids.product_id', '=', self.product_id.id),
             ('state', '=', 'draft')])
