            ...
            if int(schedule.dayofweek) == day_of_week:
                ...
                if permission_request:
                    if permission_request.permission_type == "beginning":
                        ...
                        if (
                            permission_request.start_time.time()
                            == fields.Datetime.from_string(schedule.hour_from).time()
                        ):
                            work_start += 2
                        elif (
                            permission_request.end_time.time()
                            == fields.Datetime.from_string(schedule.hour_to).time()
                        ):
                            work_end -= 2
                break
