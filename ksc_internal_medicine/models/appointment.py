...
class AccountMove(models.Model):
    ...
    clinic = fields.Selection(selection_add=[('medicine', 'Medicine')])
...
class KSCServiceLine(models.Model):
    ...
    medicine_appt_id = fields.Many2one(
        'ksc.medicine.appointment', ondelete="cascade", string='Appointment')

    def _compute_patient_id(self):
        super(KSCServiceLine, self)._compute_patient_id()
        for rec in self:
            if rec.medicine_appt_id:
                rec.patient_id = rec.medicine_appt_id.patient_id

    def _compute_clinic_name(self):
        super(KSCServiceLine, self)._compute_clinic_name()
        for rec in self:
            if rec.medicine_appt_id:
                rec.clinic_name = rec.medicine_appt_id.get_clinic_name()
...
class KscDiseasesLine(models.Model):
    ...
    medicine_appt_id = fields.Many2one(
        'ksc.medicine.appointment', ondelete="cascade", string='Medicine Appointment')
...
class KscMedicineAppointment(models.Model):
    ...
    service_line_ids = fields.One2many('ksc.service.line', 'medicine_appt_id', string='Service Line',copy=False)
    diseases_ids = fields.One2many('ksc.diseases.line', 'medicine_appt_id')
    clinic_name = fields.Char(default="medicine")
    # diseases_ids = fields.One2many('ksc.diseases', 'medicine_appt_id')

    def get_receptionist_clinic_group(self):
        return "ksc_internal_medicine.ksc_medicine_receptionist"

    def get_nurse_clinic_group(self):
        return "ksc_internal_medicine.ksc_medicine_nurse"

    def get_manager_clinic_group(self):
        return "ksc_internal_medicine.ksc_medicine_manager"
...
class MedicinePatientWizard(models.TransientModel):
    _inherit = "patient.wizard"

    def print_medicine_info_for_this_patient(self):
        domain = [('patient_id', '=', self.patient_id.id)]
        lab_ids = self.env['ksc.medicine.appointment'].search(
            domain, order="start_date asc").ids
        if lab_ids:
            return self.env.ref(
                'ksc_internal_medicine.medicine_patient_file_action').report_action([lab_ids])
        else:
