...
class AccountInvoice(models.Model):
    ...
    request_id = fields.Many2one(
        'ksc.laboratory.request', string='Lab Request', copy=False, ondelete='restrict')
...
class Physician(models.Model):
    ...
    def _rec_count(self):
        ...
        for record in self.with_context(active_test=False):
            record.lab_request_count = Labrequest.search_count(
                [('physician_id', '=', record.id)])
            record.lab_result_count = Labresult.search_count(
                [('physician_id', '=', record.id)])

    lab_request_count = fields.Integer(
        compute='_rec_count', string='# Lab Request')
    lab_result_count = fields.Integer(
        compute='_rec_count', string='# Lab Result')
...
class kscPatient(models.Model):
    ...
    request_ids = fields.One2many(
        'ksc.laboratory.request', 'patient_id', string='Lab Requests')
    test_ids = fields.One2many(
        'patient.laboratory.test', 'patient_id', string='Tests')
    request_count = fields.Integer(
        compute='_rec_count', string='# Lab Requests')
