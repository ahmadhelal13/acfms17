...
class PatientLabTestLine(models.Model):
    ...
    instruction = fields.Char(related="test_id.ksc_instruction", string="Special Instructions", readonly=True)
    ...
    company_id = fields.Many2one("res.company", ondelete="restrict", string="Company", related="request_id.company_id")
    is_invoiced = fields.Boolean()

    def _compute_test_state(self):
        for rec in self:
            test = self.env["patient.laboratory.test"].search([("test_id", "=", rec.test_id.id)], order="create_date desc", limit=1)
            if test:
    ...
    def prepare_test_result_data(self):
        ...
        if self.test_id.structure_selection == "parameter":
            lines = self.test_id.get_line_by_age_and_gender(self.request_id.patient_id)
            if lines:
                for line in lines:
                    result_ids.append(
                        (
        ...
        if self.test_id.has_two_structure:
            lines = self.test_id.get_line_by_age_and_gender_2(self.request_id.patient_id)
        ...
        if self.test_id.has_three_structure:
            for line_3 in self.test_id.parameter_ids_3:
                result_ids_3.append((0, 0, {"parameter_id": line_3.id, "name": line_3.name, "normal_range": line_3.normal_range_3, "remark": line_3.remark}))
        res = {
...
class LaboratoryRequest(models.Model):
    ...
    date = fields.Datetime("Date", widget="datetime:format('%d/%m/%Y %I:%M %p')", readonly=True, default=lambda self: fields.Datetime.now(), tracking=True)
    state = fields.Selection(
        [("draft", "Draft"), ("requested", "Requested"), ("accepted", "Accepted"), ("in_progress", "In Progress"), ("cancel", "Cancel"), ("done", "Done")],
        string="State",
        readonly=True,
        default="draft",
        tracking=True,
    )
    patient_id = fields.Many2one("res.partner", domain="[('is_patient','=',True)]", string="Patient", required=True, ondelete="restrict", tracking=True)
    physician_id = fields.Many2one(
        "res.partner", domain="[('is_physician','=',True)]", string="Prescribing Doctor", help="Doctor who Request the lab test.", ondelete="restrict", tracking=True
    )
    invoice_id = fields.Many2one("account.move", string="Invoice", copy=False)
    lab_bill_id = fields.Many2one("account.move", string="Vendor Bill", copy=False)
    line_ids = fields.One2many("laboratory.request.line", "request_id", string="Lab Test Line")
    ...
    laboratory_id = fields.Many2one("ksc.laboratory", ondelete="restrict", string="Laboratory")
    sample_ids = fields.One2many("ksc.patient.laboratory.sample", "request_id", string="Test Samples")
    laboratory_group_id = fields.Many2one("laboratory.group", ondelete="set null", string="Laboratory Group")
    invoice_state = fields.Selection([("paid", "Paid"), ("not_paid", "Not Paid")], compute="_compute_invoice_state")
    has_credit_note = fields.Selection([("has_credit", "Has Credit"), ("has_no_credit", "Has No Credit Note")], compute="_compute_has_credit_note")
    is_invoiced = fields.Boolean()
    ...
    def create(self, vals):
        if vals.get("name", "New") == "New":
            vals["name"] = self.env["ir.sequence"].next_by_code("ksc.laboratory.request") or "New"
    ...
    def _compute_invoice_state(self):
        for rec in self:
            # Check if rec.date exists and is a valid date/datetime
            if not rec.date:
                rec.invoice_state = "not_paid"
                continue

            # Convert to date if it's a datetime
            target_date = rec.date.date() if hasattr(rec.date, "date") else rec.date

            invoice_ids = rec.env["account.move"].search(
                [
                    ("appointment_name", "=", rec.name),
                    ("clinic", "=", rec.get_clinic_name()),
    ...
    def get_clinic_name(self):
        name = self._name.replace("ksc.", "").replace(".request", "").replace(".", "_")
        return name
    ...
    def button_cancel(self):
        test_ids = self.env["patient.laboratory.test"].search([("request_id", "=", self.id)])
        if test_ids:
    ...
    def print_report(self):
        ...
        payment_ids = self.env["account.payment"].search([("clinic", "=", "laboratory"), ("date", "=", fields.Date.today()), ("create_uid", "=", self.env.uid)])
        if payment_ids:
