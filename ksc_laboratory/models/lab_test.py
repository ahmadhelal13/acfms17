...
class LabTestResult(models.Model):
    ...
    culture_growth_result = fields.Selection([("sensitive", "Sensitive"), ("intermediate_sensitive", "Intermediate Sensitive"), ("resistant", "Resistant")], string="Culture 1")
    culture_growth_result_2 = fields.Selection([("sensitive", "Sensitive"), ("intermediate_sensitive", "Intermediate Sensitive"), ("resistant", "Resistant")], string="Culture 2")
...
urine_result_list = [
    ...
    ("2", "Mucus threads"),
    ("3", "Yeast Cells"),
    ("4", "Budding Yeast Cells"),
    ("5", "Monilia"),
    ("6", "Spermatozoa"),
    ("7", "Oval Fat Bodies"),
    ("8", "Trichomonas vaginalis trophozoite"),
    ("9", "Scistosoma haematobium ova"),
    ("10", "Cotton Fiber"),
    ("11", "Talcum Powder"),
    ("12", "Starch"),
    ("13", "Hair"),
    ("14", "Talcum Powder"),
    ("16", "Calcium carbonates"),
    ("17", "Calcium phosphates"),
    ("18", "Styruvites"),
    ("19", "Cholesterol"),
    ("20", "Cystine"),
    ("21", "Leucine"),
    ("22", "Tyrosine"),
    ("23", "Sulfa"),
    ("24", "Ammonium biurates"),
    ("25", "Acyclovir"),
    ("26", "Amoxicillin"),
    ("27", "Hyaline casts"),
    ("28", "Waxy casts"),
    ("29", "Coarse granular casts"),
    ("30", "Fine granular casts"),
    ("31", "Leukocyte casts"),
    ...
    ("42", "Neutrophil casts"),
    ("43", "Oxalate casts"),
    ("44", "Renal Epithelial casts"),
    ("45", "Renal Tubular casts"),
    ("46", "Uric Acid casts"),
    ("47", "Fat Bodies"),
    ("48", "Feces"),
    ("49", "Clue cell"),
    ("50", "Triple phosphates"),
    ("51", "Amorphous urates"),
    ("52", "Amorphous phosphates"),
    ("53", "Calcium oxalates"),
    ("54", "Uric acids"),
    ("55", "Triple phosphates"),
]

urine_shape_list = [("1", "present"), ("2", "+"), ("3", "++"), ("4", "+++"), ("5", "++++")]

other_shape_stool_list = [
    ("1", "present"),
    ("2", "ova seen"),
    ("3", "fertilized ova seen"),
    ("4", "unfertilized ova seen"),
    ("5", "cysts seen"),
    ("6", "trophozoites seen"),
    ("7", "adult seen"),
    ("8", "adult female seen"),
    ...
    ("12", "oocysts seen"),
    ("13", "larvae seen"),
    ("14", "filariform larvae seen"),
    ("15", "rhabditiform larvae seen"),
]

semen_head_defects_list1 = [("1", "Double"), ("2", "Giant"), ("3", "Amorphous"), ("4", "Pin"), ("5", "Tapered"), ("6", "Constricted")]

semen_head_defects_list2 = [("1", "/ Double"), ("2", "/ Giant"), ("3", "/ Amorphous"), ("4", "/ Pin"), ("5", "/ Tapered"), ("6", "/ Constricted")]

semen_neck_midpiece_defects_list1 = [("1", "Asymmetric"), ("2", "Bent"), ("3", "Thin"), ("4", "Thick"), ("5", "Irregular"), ("6", "Cytoplasmic Droplet")]

semen_neck_midpiece_defects_list2 = [("1", "/ Asymmetric"), ("2", "/ Bent"), ("3", "/ Thin"), ("4", "/ Thick"), ("5", "/ Irregular"), ("6", "/ Cytoplasmic Droplet")]


semen_tail_defects_list1 = [("1", "Doubled"), ("2", "Coiled"), ("3", "Short"), ("4", "Broken")]

semen_tail_defects_list2 = [("1", "/ Doubled"), ("2", "/ Coiled"), ("3", "/ Short"), ("4", "/ Broken")]
...
growth_of_list = [
    ...
    ("17", "Group 'C' Streptococci"),
    ("18", "Group 'D' Streptococci"),
    ("19", "Group 'F' Streptococci"),
    ("20", "Group 'G' Streptococci"),
    ("21", "Streptococcus pyogenes"),
    ("22", "Streptococcus agalactiae"),
    ("23", "Streptococcus pneumoniae"),
    ("24", "Group 'D' Enterococcus"),
    ("25", "Group 'D' Non-Enterococcus"),
]
# ======================================================================================
# ======================================================================================
# ======================================================================================
...
class PatientLabTest(models.Model):
    ...
    test_id = fields.Many2one("ksc.lab.test", string="Test", required=True, ondelete="restrict", tracking=True)
    patient_id = fields.Many2one("res.partner", string="Patient", required=True, ondelete="restrict", tracking=True)
    user_id = fields.Many2one("res.users", string="Lab User", default=lambda self: self.env.user, tracking=True)
    physician_id = fields.Many2one("res.partner", string="Prescribing Doctor", help="Doctor who requested the test", ondelete="restrict", tracking=True)
    processed_by = fields.Many2one("res.users", "Processed By", readonly=True, tracking=True)
    diagnosis = fields.Text(string="Diagnosis", tracking=True)
    has_multi_dev = fields.Boolean(readonly=True, compute="compute_has_multi_device")
    device_id = fields.Selection([("1", "Vitros (XT 7600 System Integrated)"), ("2", "Snibe (Maglumi X3)")], "Device")
    ...
    # result one
    result_ids = fields.One2many("lab.test.result", "patient_lab_id", tracking=True)
    culture = fields.Selection(
        [
            ...
            ("5", "10⁴ - 10⁵ growth of"),
            ("6", "> 10⁵ growth of"),
            ("7", "No growth after 48 hrs of incubation"),
            ("8", "No significant bacterial growth"),
            ("9", "Growth of normal bacterial flora"),
            ("10", "No pathogenic organism isolated"),
            ("11", "No Salmonella spp. nor Shigella spp. isolated after 48 hours of incubation."),
        ],
        string="Culture 1",
        tracking=True,
    )
    growth_of = fields.Selection(growth_of_list, string="Growth Of", tracking=True)
    ...
    growth_of_2 = fields.Selection(growth_of_list, string="Growth Of", tracking=True)
    ...
    growth_of_3 = fields.Selection(growth_of_list, string="Growth Of", default="1", tracking=True)
    has_2_bactria = fields.Boolean("Has Two Culture", default=False, tracking=True)
    has_3_bactria = fields.Boolean("Has Three Culture", tracking=True)
    fungal = fields.Selection(
        [("1", "No fungal growth obtained"), ("2", "Scanty growth of"), ("3", "Moderate growth of"), ("4", "Heavy growth of")], string="Fungal Culture", tracking=True
    )
    candida = fields.Selection([("1", "Candida albicans"), ("2", "Candida species.")], string="Candida", tracking=True)
    ...
    is_customize_and_culture_growth = fields.Boolean(compute="func_is_customize_and_culture_growth", default=False)
    ...
    is_patient_and_control = fields.Boolean(compute="func_is_patient_and_control", default=False)
    is_inr = fields.Boolean(compute="func_is_inr", default=False)
    is_normal_range_2 = fields.Boolean(compute="func_is_normal_range_2", default=False)
    ...
    is_brucella_rose = fields.Boolean(compute="func_is_brucella_rose", default=False)
    is_blood_group = fields.Boolean(compute="func_is_blood_group", default=False)
    is_fixed_table = fields.Boolean(compute="func_fixed_table", default=False)
    is_special_test_and_sprcial_result_and_patient_control = fields.Boolean(compute="func_is_special_test_and_sprcial_result_and_patient_control", default=False)
    # end result one

    # result two
    result_ids_2 = fields.One2many("lab.test.result.two", "patient_lab_id", tracking=True)
    has_two_result = fields.Boolean(compute="func_has_two_result", default=False)
    is_patient_and_control_2 = fields.Boolean(compute="func_is_patient_and_control_2", default=False)
    is_polar_2 = fields.Boolean(compute="func_is_polar_2", default=False)
    is_custom_reference_2 = fields.Boolean(compute="func_is_custom_reference_2", default=False)
    # end result two

    # result three
    result_ids_3 = fields.One2many("lab.test.result.three", "patient_lab_id", tracking=True)
    has_three_result = fields.Boolean(compute="func_has_three_result", default=False)
    is_min_sec_3 = fields.Boolean(compute="func_is_min_sec_3", default=False)
    is_normal_range_3 = fields.Boolean(compute="func_is_normal_range_3", default=False)
    # end result three

    # special result
    malaria_result_ids = fields.One2many("malaria.results", "patient_lab_id", tracking=True)
    grain_stain_result_ids = fields.One2many("gram.stain.results", "patient_lab_id", tracking=True)
    # end special result

    # special test
    # urine routine
    is_urine_routine_test = fields.Boolean(compute="func_is_urine_routine_test", default=False)
    color_urine = fields.Selection(
        [("1", "Straw"), ("2", "Light Yellow"), ("3", "Yellow"), ("4", "Dark Yellow"), ("5", "Amber"), ("6", "Red"), ("7", "Brown"), ("8", "Black")], string="Color", tracking=True
    )
    appearance_urine = fields.Selection([("1", "Clear"), ("2", "Slightly Turbid"), ("3", "Turbid")], string="Appearance", tracking=True)

    specific_gravity_urine = fields.Selection(
        [("1", "1.000"), ("2", "1.005"), ("3", "1.010"), ("4", "1.015"), ("5", "1.020"), ("6", "1.025"), ("7", "1.030")], string="Specific Gravity (SG)", tracking=True
    )
    ph_urine = fields.Selection([("1", "5.0"), ("2", "5.5"), ("3", "6.0"), ("4", "6.5"), ("5", "7.0"), ("6", "8.0"), ("7", "9.0")], string="pH", tracking=True)
    glucose_urine = fields.Selection([("1", "Normal"), ("2", "2.8"), ("3", "5.5"), ("4", "17"), ("5", "55")], string="Glucose (GLU)", tracking=True)
    lecucocytes_urine = fields.Selection([("1", "Negative"), ("2", "10"), ("3", "25"), ("4", "75"), ("5", "500")], string="Leucocytes (LEU)", tracking=True)
    nitrite_urine = fields.Selection([("1", "Negative"), ("2", "Positive")], string="Nitrite (NIT)", tracking=True)
    protien_urine = fields.Selection([("1", "Negative"), ("2", "0.3"), ("3", "1"), ("4", "5")], string="Protein (PRO)nce", tracking=True)
    ketones_urine = fields.Selection([("1", "Negative"), ("2", "1.5"), ("3", "5"), ("4", "15")], string="Ketones (KET)", tracking=True)
    urobilinogen_urine = fields.Selection([("1", "Normal"), ("2", "17"), ("3", "51"), ("4", "102"), ("5", "203")], string="Urobilinogen (URO)", tracking=True)
    bilirubin_urine = fields.Selection([("1", "Negative"), ("2", "17"), ("3", "51"), ("4", "103"), ("5", "+"), ("6", "++"), ("7", "+++")], string="Bilirubin (BIL)", tracking=True)
    blood_urine = fields.Selection([("1", "Negative"), ("2", "5"), ("3", "10"), ("4", "50"), ("5", "250")], string="Blood (BLD)", tracking=True)
    # normal range
    specific_gravity_urine_normal_range = fields.Char("Specific Gravity Normal Range", readonly=True)
    ph_urine_normal_range = fields.Char("PH Normal Range", readonly=True)
    glucose_urine_normal_range = fields.Char("Glucose Normal Range", readonly=True)
    lecucocytes_urine_normal_range = fields.Char("Lecucocytes Normal Range", readonly=True)
    nitrite_urine_normal_range = fields.Char("Nitrite Normal Range", readonly=True)
    protien_urine_normal_range = fields.Char("Protien Normal Range", readonly=True)
    ketones_urine_normal_range = fields.Char("Ketones Normal Range", readonly=True)
    urobilinogen_urine_normal_range = fields.Char("Urobilinogen Normal Range", readonly=True)
    bilirubin_urine_normal_range = fields.Char("Bilirubin Normal Range", readonly=True)
    blood_urine_normal_range = fields.Char("Blood Normal Range", readonly=True)
    # UOM of normal range
    glucose_urine_UOM = fields.Many2one("ksc.lab.test.uom", string="Glucose UOM", readonly=True, related="test_id.glucose_urine_UOM")
    lecucocytes_urine_UOM = fields.Many2one("ksc.lab.test.uom", string="Leucocytes UOM", readonly=True, related="test_id.lecucocytes_urine_UOM")
    protien_urine_UOM = fields.Many2one("ksc.lab.test.uom", string="Protein UOM", readonly=True, related="test_id.protien_urine_UOM")
    ketones_urine_UOM = fields.Many2one("ksc.lab.test.uom", string="Ketones UOM", readonly=True, related="test_id.ketones_urine_UOM")
    urobilinogen_urine_UOM = fields.Many2one("ksc.lab.test.uom", string="Urobilinogen UOM", readonly=True, related="test_id.urobilinogen_urine_UOM")
    bilirubin_urine_UOM = fields.Many2one("ksc.lab.test.uom", string="Bilirubin UOM", readonly=True, related="test_id.bilirubin_urine_UOM")
    blood_urine_UOM = fields.Many2one("ksc.lab.test.uom", string="Blood UOM", readonly=True, related="test_id.blood_urine_UOM")

    white_blood_cells_urine = fields.Char("White Blood Cells (WBC)", tracking=True)
    red_blood_cells_urine = fields.Char("Red Blood Cells (RBC)", tracking=True)
    epithelial_cells_urine = fields.Selection([("1", "Nil"), ("2", "+"), ("3", "++"), ("4", "+++"), ("5", "++++")], string="Epithelial Cells", tracking=True)
    bacteria_urine = fields.Selection([("1", "Nil"), ("2", "+"), ("3", "++"), ("4", "+++"), ("5", "++++")], string="Bacteria", tracking=True)
    ...
    crystals_shape_urine = fields.Selection(urine_shape_list, string="Crystal Quantity", tracking=True)
    casts_urine = fields.Selection(
        [
            ("1", "Nil"),
            ("2", "Hyaline casts"),
            ("3", "Waxy casts"),
            ...
            ("13", "Neutrophil casts"),
            ("18", "Oxalate casts"),
            ("19", "Renal Epithelial casts"),
            ("20", "Renal Tubular casts"),
            ("21", "Uric Acid casts"),
        ],
        string="Casts",
        tracking=True,
    )
    casts_shape_urine = fields.Selection(urine_shape_list, string="Casts Quantity", tracking=True)
    ...
    appearance_urine_without_micor = fields.Selection([("1", "Clear"), ("2", "Slightly Turbid"), ("3", "Turbid")], string="Appearance", tracking=True)

    specific_gravity_urine_without_micor = fields.Selection(
        [("1", "1.000"), ("2", "1.005"), ("3", "1.010"), ("4", "1.015"), ("5", "1.020"), ("6", "1.025"), ("7", "1.030")], string="Specific Gravity (SG)", tracking=True
    )
    ph_urine_without_micor = fields.Selection([("1", "5.0"), ("2", "5.5"), ("3", "6.0"), ("4", "6.5"), ("5", "7.0"), ("6", "8.0"), ("7", "9.0")], string="pH", tracking=True)
    glucose_urine_without_micor = fields.Selection([("1", "Normal"), ("2", "2.8"), ("3", "5.5"), ("4", "17"), ("5", "55")], string="Glucose (GLU)", tracking=True)
    lecucocytes_urine_without_micor = fields.Selection([("1", "Negative"), ("2", "10"), ("3", "25"), ("4", "75"), ("5", "500")], string="Leucocytes (LEU)", tracking=True)
    nitrite_urine_without_micor = fields.Selection([("1", "Negative"), ("2", "Positive")], string="Nitrite (NIT)", tracking=True)
    protien_urine_without_micor = fields.Selection([("1", "Negative"), ("2", "0.3"), ("3", "1"), ("4", "5")], string="Protein (PRO)", tracking=True)
    ketones_urine_without_micor = fields.Selection([("1", "Negative"), ("2", "1.5"), ("3", "5"), ("4", "15")], string="Ketones (KET)", tracking=True)
    urobilinogen_urine_without_micor = fields.Selection([("1", "Normal"), ("2", "17"), ("3", "51"), ("4", "102"), ("5", "203")], string="Urobilinogen (URO)", tracking=True)
    bilirubin_urine_without_micor = fields.Selection(
        [("1", "Negative"), ("2", "17"), ("3", "51"), ("4", "103"), ("5", "+"), ("6", "++"), ("7", "+++")], string="Bilirubin (BIL)", tracking=True
    )
    blood_urine_without_micor = fields.Selection([("1", "Negative"), ("2", "5"), ("3", "10"), ("4", "50"), ("5", "250")], string="Blood (BLD)", tracking=True)
    # normal range
    specific_gravity_urine_without_mico_normal_range = fields.Char("Specific Gravity Normal Range", readonly=True)
    ph_urine_without_mico_normal_range = fields.Char("PH Normal Range", readonly=True)
    glucose_urine_without_mico_normal_range = fields.Char("Glucose Normal Range", readonly=True)
    lecucocytes_urine_without_mico_normal_range = fields.Char("Lecucocytes Normal Range", readonly=True)
    nitrite_urine_without_mico_normal_range = fields.Char("Nitrite Normal Range", readonly=True)
    protien_urine_without_mico_normal_range = fields.Char("Protien Normal Range", readonly=True)
    ketones_urine_without_mico_normal_range = fields.Char("Ketones Normal Range", readonly=True)
    urobilinogen_urine_without_mico_normal_range = fields.Char("Urobilinogen Normal Range", readonly=True)
    bilirubin_urine_without_mico_normal_range = fields.Char("Bilirubin Normal Range", readonly=True)
    blood_urine_without_mico_normal_range = fields.Char("Blood Normal Range", readonly=True)
    # UOM of urine routine without micor normal range

    glucose_urine_UOM_without_mico = fields.Many2one("ksc.lab.test.uom", string="Glucose UOM", readonly=True, related="test_id.glucose_urine_UOM_without_mico")
    lecucocytes_urine_UOM_without_mico = fields.Many2one("ksc.lab.test.uom", string="Leucocytes UOM", readonly=True, related="test_id.lecucocytes_urine_UOM_without_mico")
    protien_urine_UOM_without_mico = fields.Many2one("ksc.lab.test.uom", string="Protien UOM", readonly=True, related="test_id.protien_urine_UOM_without_mico")
    ketones_urine_UOM_without_mico = fields.Many2one("ksc.lab.test.uom", string="Ketones UOM", readonly=True, related="test_id.ketones_urine_UOM_without_mico")
    urobilinogen_urine_UOM_without_mico = fields.Many2one("ksc.lab.test.uom", string="Urobilinogen UOM", readonly=True, related="test_id.urobilinogen_urine_UOM_without_mico")
    bilirubin_urine_UOM_without_mico = fields.Many2one("ksc.lab.test.uom", string="Bilirubin UOM", readonly=True, related="test_id.bilirubin_urine_UOM_without_mico")
    blood_urine_UOM_without_mico = fields.Many2one("ksc.lab.test.uom", string="Blood UOM", readonly=True, related="test_id.blood_urine_UOM_without_mico")

    white_blood_cells_urine = fields.Char("White Blood Cells (WBC)", tracking=True)
    red_blood_cells_urine = fields.Char("Red Blood Cells (RBC)", tracking=True)
    # end of urine without micor
    # stool routine
    is_stool_routine_test = fields.Boolean(compute="func_is_stool_routine_test", default=False)
    ...
    color_stool = fields.Selection(
        [("1", "Yellow"), ("2", "Yellowish Green"), ("3", "Green"), ("4", "Yellowish Brown"), ("5", "Brown"), ("6", "Dark Brown"), ("7", "Black"), ("8", "White")],
        string="Color",
        tracking=True,
    )
    consistency_stool = fields.Selection(
        [("1", "Soft-formed"), ("2", "Hard-formed"), ("3", "Semi-formed"), ("4", "Mucoid"), ("5", "Watery"), ("6", "Liquid")], string="Consistency", tracking=True
    )
    blood_stool = fields.Selection([("1", "Nil"), ("2", "Present")], string="Blood", tracking=True)
    mucus_stool = fields.Selection([("1", "Nil"), ("2", "Present")], string="Mucus", tracking=True)

    white_blood_cells_stool = fields.Char("White Blood Cells (WBC)", tracking=True)
    red_blood_cells_stool = fields.Char("Red Blood Cells (RBC)", tracking=True)
    yeast_cell_stool = fields.Selection([("1", "Nil"), ("2", "+"), ("3", "++"), ("4", "+++"), ("5", "++++")], string="Yeast Cells", tracking=True)
    ...
    parasites_stool = fields.Selection(
        [
            ...
            ("11", "Hymenolepis nana"),
            ("12", "Balantidium coli"),
            ("13", "Opisthorchis viverrini"),
            ("14", "Clonorchis sinensis"),
            ("15", "Capillaria philippinensis"),
            ("16", "Taenia spp."),
            ("17", "Diphyllobothrium latum"),
            ("18", "Hymenolepis diminuta"),
            ("19", "Trichostongylus spp."),
            ("20", "Nanophyetus salmincola"),
            ("21", "Paragonimus westermani"),
            ("22", "Dipylidium caninum"),
            ("23", "Echinostoma spp."),
            ("24", "Fasciolopsis buski"),
            ("25", "Fasciola hepatica"),
            ("26", "Schistosoma japonicum"),
            ("27", "Schistosoma mekongi"),
            ("28", "Schistosoma haematobium"),
            ("29", "Schistosoma intercalatum"),
            ("30", "Schistosoma mansoni"),
            ("31", "Iodamoeba butschlii"),
            ("32", "Entamoeba polecki"),
            ("33", "Entamieba hartmanni"),
            ("34", "Entamoeba coli"),
    ...
    parasites_shape_stool = fields.Selection(other_shape_stool_list, string="Shape", tracking=True)
    ...
    other_1_shape_stool = fields.Selection(other_shape_stool_list, string="Shape", tracking=True)
    ...
    other_2_shape_stool = fields.Selection(other_shape_stool_list, string="Shape", tracking=True)
    ...
    other_3_shape_stool = fields.Selection(other_shape_stool_list, string="Shape", tracking=True)
    ...
    other_4_shape_stool = fields.Selection(other_shape_stool_list, string="Shape", tracking=True)
    # end stool routine

    # semins test
    is_semen_test = fields.Boolean(compute="func_is_semen_test", default=False, tracking=True)
    # --------------semen analysis-------------
    semen_time_collected = fields.Char("Time Collected", tracking=True)
    semen_abstination = fields.Selection(
        [("1", "2 Days"), ("2", "3 Days"), ("3", "4 Days"), ("4", "5 Days"), ("5", "6 Days"), ("6", "7 Days"), ("7", ">7 Days")], "Abstination", tracking=True
    )
    # ------------physical exam part------------
    semen_volume = fields.Float("Volume", tracking=True)
    semen_color = fields.Selection(
        [
            ("pearly_white", "Pearly White"),
    ...
    semen_liquifaction_time = fields.Integer("Liquifaction Time", tracking=True)
    ...
    semen_viscosity = fields.Selection([("normal", "Normal"), ("thin", "Thin"), ("thick", "Thick"), ("viscous", "Viscous")], "Viscosity", tracking=True)
    semen_ph = fields.Selection([("1", "7.0"), ("2", "7.5"), ("3", "8.0"), ("4", "8.5"), ("5", "9.0")], "pH", tracking=True)
    # ------------micro exam part------------
    ...
    semen_motility_after_30_min = fields.Char("Motile After 30 minutes", tracking=True)
    semen_non_motility_after_30_min = fields.Char("Non-Motile After 30 minutes", tracking=True)
    semen_motility_after_1_hour = fields.Char("Motile After 1 hour", tracking=True)
    semen_non_motility_after_1_hour = fields.Char("Non-Motile After 1 hour", tracking=True)
    semen_motility_after_2_hour = fields.Char("Motile After 2 hour", tracking=True)
    semen_non_motility_after_2_hour = fields.Char("Non-Motile After 2 hour", tracking=True)

    semen_progressive_activity_after_30_min = fields.Char("Activity Progressive After 30 minutes", tracking=True)
    semen_sluggish_activity_after_30_min = fields.Char("Activity Sluggish After 30 minutes", tracking=True)
    semen_progressive_activity_after_1_hour = fields.Char("Activity Progressive After 1 hour", tracking=True)
    semen_sluggish_activity_after_1_hour = fields.Char("Activity Sluggish After 1 hour", tracking=True)
    semen_progressive_activity_after_2_hour = fields.Char("Activity Progressive After 2 hour", tracking=True)
    semen_sluggish_activity_after_2_hour = fields.Char("Activity Sluggish After 2 hour", tracking=True)

    # -----chemecal section-------------
    semen_white_blood_cells = fields.Char("White Blood Cells (WBC)", tracking=True)
    ...
    semen_comments = fields.Selection(
        [
            ("1", "AZOOSPERMIA. (No spermatozoa seen even after centrifugation.)"),
            ("2", "Occasional motile and non-motile spermatozoa seen."),
    ...
    semen_head_defects = fields.Selection(semen_head_defects_list1, "Head defects 1", tracking=True)
    semen_head_defects_2 = fields.Selection(semen_head_defects_list2, "Head defects 2", tracking=True)
    semen_head_defects_3 = fields.Selection(semen_head_defects_list2, "Head defects 3", tracking=True)
    semen_head_defects_4 = fields.Selection(semen_head_defects_list2, "Head defects 4", tracking=True)
    ...
    # Mycoplasma test
    is_mycoplasma_test = fields.Boolean(compute="func_is_mycoplasma_test", default=False, tracking=True)

    # culture test
    is_culture_test = fields.Boolean(compute="func_is_culture_test", default=False, tracking=True)

    mycoplasma_hominis = fields.Selection(
        [
            ("1", "No growth after 48 hours of incubation"),
            ("2", "≧10⁴ growth"),
        ],
        "Mycoplasma hominis",
        tracking=True,
    ...
    # wet film test
    is_wet_film_test = fields.Boolean(compute="func_is_wet_film_test", default=False, tracking=True)
    white_blood_cells_wet_film = fields.Char("White Blood Cells (WBC)", tracking=True)
    red_blood_cells_wet_film = fields.Char("Red Blood Cells (RBC)", tracking=True)
    epithelial_cells_wet_film = fields.Selection(
        [("1", "Nil"), ("2", "Not seen"), ("3", "Present"), ("4", "Occasional"), ("5", "Few"), ("6", "+"), ("7", "++"), ("8", "+++"), ("9", "++++")],
        string="Epithelial Cells",
        tracking=True,
    )
    ...
    finding_wet_1 = fields.Boolean("No Trichomonas vaginalis seen", tracking=True)
    ...
    celiac_quick_test = fields.Selection([("negative", "NEGATIVE"), ("positive", "POSITIVE")], string="CELIAC QUICK TEST", tracking=True)
    # end CELIAC test
    ...
    date_analysis = fields.Date(string="Test Date", default=fields.Date.context_today)
    released_date = fields.Datetime(string="Released Date", readonly=True)
    request_id = fields.Many2one("ksc.laboratory.request", string="Lab Request", ondelete="restrict")
    ...
    sample_ids = fields.Many2many("ksc.patient.laboratory.sample", "test_lab_sample_rel", "test_id", "sample_id", string="Test Samples")
    ...
    def get_report_url(self):
        ...
        for rec in self:
            url = base_url + "/report/pdf/ksc_laboratory.lab_result_template/%s" % (rec.id)
            rec.report_url = url
            print("-------------------------------")
            print(url)
            print("-------------------------------")
    ...
    # aso
    is_aso_test = fields.Boolean(compute="func_is_aso_test", default=False, tracking=False)
    ...
    # crp
    is_crp_test = fields.Boolean(compute="func_is_crp_test", default=False, tracking=False)
    ...
    # rf
    is_rf_test = fields.Boolean(compute="func_is_rf_test", default=False, tracking=False)
    ...
    # Gonorrhea
    is_gonorrhea_test = fields.Boolean(compute="func_is_gonorrhea_test", default=False, tracking=False)
    ...
    # celiac
    is_celiac_test = fields.Boolean(compute="func_is_celiac_test", default=False, tracking=False)
    ...
    # Alcohol
    is_alcohol_test = fields.Boolean(compute="func_is_alcohol_test", default=False, tracking=False)
    ...
    # drugs
    is_drugs_test = fields.Boolean(compute="func_is_drugs_test", default=False, tracking=False)
