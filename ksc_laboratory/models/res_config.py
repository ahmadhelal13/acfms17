...
class ResCompany(models.Model):
    ...
    laboratory_usage_location = fields.Many2one("stock.location", string="Usage Location for Consumed Laboratory Test Material.")
    laboratory_stock_location = fields.Many2one("stock.location", string="Stock Location for Consumed Laboratory Test Material")
    ...
    laboratory_journal_ids = fields.Many2many("account.journal", "company_laboratory_journal")

    laboratory_calendar_weekends = fields.Boolean(string="Lab Show weekends?", default=True)
    laboratory_calendar_weeknumber = fields.Boolean(string="Lab Show week number?", default=True)
    laboratory_calendar_weekday = fields.Selection(selection=WEEKDAYS, string="Lab First day of week?", required=True, default="0")
    laboratory_calendar_allow_overlap = fields.Boolean(string="Lab Allow events overlap?", default=True)
    laboratory_calendar_disable_dragging = fields.Boolean(string="Lab Disable drag and drop?", default=False)
    laboratory_calendar_disable_resizing = fields.Boolean(string="Lab Disable resizing?", default=False)
    laboratory_calendar_snap_minutes = fields.Integer(string="Lab Default minutes when creating and resizing", default=15)
    laboratory_calendar_slot_minutes = fields.Integer(string="Lab Minutes per row", default=30)
    laboratory_calendar_min_time = fields.Float(string="Lab Calendar time range from", default=0.0)
    laboratory_calendar_max_time = fields.Float(string="Lab Calendar time range to", default=24.0)
    laboratory_calendar_start_time = fields.Float(string="Lab Start time", default=6.0)


class ResConfigSettings(models.TransientModel):
    _inherit = "res.config.settings"

    laboratory_usage_location = fields.Many2one(
        "stock.location",
    ...
    laboratory_stock_location = fields.Many2one(
        "stock.location",
    ...
    laboratory_journal_ids = fields.Many2many("account.journal", related="company_id.laboratory_journal_ids", readonly=False)
    ...
    laboratory_calendar_start_time = fields.Float(string="Lab Start time", related="company_id.laboratory_calendar_start_time", readonly=False)
