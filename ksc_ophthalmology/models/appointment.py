...
class AccountMove(models.Model):
    ...
    clinic = fields.Selection(
        selection_add=[('ophthalmology', 'Ophthalmology')])
...
class KSCServiceLine(models.Model):
    ...
    ophthalmology_appt_id = fields.Many2one(
        'ksc.ophthalmology.appointment', ondelete="cascade", string='Appointment')

    def _compute_patient_id(self):
        super(KSCServiceLine, self)._compute_patient_id()
        for rec in self:
            if rec.ophthalmology_appt_id:
                rec.patient_id = rec.ophthalmology_appt_id.patient_id

    def _compute_clinic_name(self):
        super(KSCServiceLine, self)._compute_clinic_name()
        for rec in self:
            if rec.ophthalmology_appt_id:
                rec.clinic_name = rec.ophthalmology_appt_id.get_clinic_name()
...
class KscDiseasesLine(models.Model):
    ...
    ophthalmology_appt_id = fields.Many2one(
        'ksc.ophthalmology.appointment', ondelete="cascade", string='Ophthalmology Appointment')
...
class KscophthalmologyAppointment(models.Model):
    ...
    clinic_name = fields.Char(default="ophthalmology")

    def get_receptionist_clinic_group(self):
        return "ksc_ophthalmology.ksc_ophthalmology_receptionist"

    def get_nurse_clinic_group(self):
        return "ksc_ophthalmology.ksc_ophthalmology_nurse"

    def get_manager_clinic_group(self):
        return "ksc_ophthalmology.ksc_ophthalmology_manager"
    ...
    def get_available(self):
        return "avalibel_in_ophthalmology"
        ...
        for values in values_list:
            if values.get('name', 'New Appointment') == 'New Appointment':
                values['name'] = self.env['ir.sequence'].next_by_code(
                    'ksc.ophthalmology.appointment') or 'New Appointment'

        return super(KscophthalmologyAppointment, self).create(values_list)
...
class OphthalmologyPatientWizard(models.TransientModel):
    _inherit = "patient.wizard"

    def print_ophthalmology_info_for_this_patient(self):
        domain = [('patient_id', '=', self.patient_id.id)]
        lab_ids = self.env['ksc.ophthalmology.appointment'].search(domain, order="start_date asc"
                                                                   ).ids
        # raise UserError(lab_ids)
        if lab_ids:
            return self.env.ref(
                'ksc_ophthalmology.ophthalmology_patient_file_action').report_action([lab_ids])
        else:
