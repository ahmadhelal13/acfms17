...
class kscPhysiotherapyExercise(models.Model):
    ...
    exercise_group_id = fields.Many2one(
        'physiotherapy.exercise.group', 'Exercise group')
...
class kscPhysiotherapyExerciseGroup(models.Model):
    ...
    name = fields.Char(string='Name', required=True,
                       help="Exercise Group Name like Knee,Ankle...")
    description = fields.Text(string='Description')
    product_id = fields.Many2one(
        'product.product', string='Service', required=True)
    exercise_ids = fields.One2many(
        'physiotherapy.exercise', 'exercise_group_id', string='Exercise')
...
class kscPhysiotherapyExerciseGroupLines(models.Model):
    ...
    exercise_ids = fields.Many2many('physiotherapy.exercise', 'rel_exercise_group_exercise_line',
                                    'exercise_id', 'exercise_group_id', string='Exercise')
    price = fields.Float(
        related='group_id.product_id.list_price', string='Price', store=True)
...
class kscPhysiotherapyNoteTemplate(models.Model):
    ...
    note_type = fields.Selection(
        [('lower', 'Lower'), ('upper', 'Upper'), ('hand', 'Hand')], string="Note Type", required=True)
...
class kscPhysiotherapySelectionNoteTemplate(models.Model):
    ...
    right_val = fields.Selection([('normal', 'Normal'), ('other', 'Other'), (
        'positive', 'Positive'), ('negative', 'Negative')], string="STRENGTH RIGHT", default='normal')
    left_val = fields.Selection([('normal', 'Normal'), ('other', 'Other'), (
        'positive', 'Positive'), ('negative', 'Negative')], string="STRENGTH LEFT", default='normal')
    selnote_type = fields.Selection(
        [('lower', 'Lower'), ('upper', 'Upper'), ('hand', 'Hand')], string="Note Type")
...
class kscPhysiotherapy(models.Model):
    ...
    def _get_years_sex(self):
        for rec in self:
            ...
            if rec.patient_id.birthday:
                ...
                age = _("%s year / %s") % (delta.years,
                                           'Male' if rec.patient_id.gender == 'male' else 'Female')
            rec.years_sex = age
        ...
        for rec in self:
            ...
            for line in self.search([('patient_id', '=', rec.patient_id.id)], order='date desc', limit=10):
                ...
                for line in line.grp_exercise_ids:
                    history += _("<tr><td>%s</td><td>%s</td><td>%s</td></tr>") % (
                        line.group_id.name, line.price, ','.join([ex.name for ex in line.exercise_ids]))
    ...
    physiotherapy_history = fields.Html(
        compute='_get_physiotherapy_history', store=True, string='Physiotherapy History', readonly=True)
    ...
    def create_invoice(self):
        ...
        invoice = self.with_context(pricelist_context).ksc_create_invoice(
            partner=self.patient_id, product_data=product_data)
    ...
    def default_get(self, fields):
        ...
        vals = []
        hand_templates = self.env['ksc.physiotherapy.note.template'].search(
            [('note_type', '=', 'hand')])
        ...
        vals = []
        lower_templates = self.env['ksc.physiotherapy.note.template'].search(
            [('note_type', '=', 'lower')])
        ...
        upper_templates = self.env['ksc.physiotherapy.note.template'].search(
            [('note_type', '=', 'upper')])
        ...
        vals = []
        lower_selection_templates = self.env['ksc.physiotherapy.selection.note.template'].search(
            [('selnote_type', '=', 'lower')])
        ...
        vals = []
        upper_selection_templates = self.env['ksc.physiotherapy.selection.note.template'].search(
            [('selnote_type', '=', 'upper')])
