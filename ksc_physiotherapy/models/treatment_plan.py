...
class TreatmentPlan(models.Model):
    ...
    name = fields.Char(string='Treatment Plan Id',
    ...
    suggested_line_ids = fields.One2many(
        'treatment.suggested.plan', 'plan_id', required=True,)
...
class TreatmentPlanSession(models.Model):
    ...
    name = fields.Char(string='Session Id', readonly=True,
    ...
    slot_time_ids = fields.Many2many(
        'slot.time', compute='_compute_slot_time_ids')
    state = fields.Selection([
        ('draft', 'Draft'),
        ('in_progress', 'In Progress'),
        ('done', 'Done'),
    ...
    def create(self, values):
        values['name'] = self.env['ir.sequence'].next_by_code(
            'treatment.plan.session') or _('New')
